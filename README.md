...
- Linux / macOS Apple Silicon は `uv run` のPEP 723 wrapperでセットアップと起動を行うようにしました。
...
- Style-Bert-VITS2 のセットアップと起動をCUDA向け `uv run` に一本化しました。
...
- アプリ起動、Style-Bert-VITS2 起動、動画生成の通常経路から `.sh` / `.bat` wrapper 依存を減らし、`uv run` を主経路にしました。
- Style-Bert-VITS2 の依存はCUDA向け `uv run` に一本化し、CPU用 wrapper を削除しました。
- KoboldCpp の手動更新用 `.bat` とサンプル起動 `.bat` を削除し、セットアップPythonとアプリ内起動に集約しました。
- `llm.json` で明示された `info_url` を上書きしないようにしました。
- 動画生成は一時 `.sh` / `.bat` ではなく、PEP 723 Python wrapperを `uv run` で起動するようにしました。
...
- Linux / macOS Apple Silicon は `.sh` ではなく `uv run EasyNovelAssistant/setup/ena.py ...` で動くようにしました。
...
`uv` と Python が利用できる環境では、共通の PEP 723 セットアップも使えます。

```bat
uv run EasyNovelAssistant\setup\ena.py setup
```

起動:

```bat
uv run EasyNovelAssistant\setup\ena.py run
```

KoboldCpp は `KoboldCpp\koboldcpp.exe` を利用します。Windows では初回セットアップ時に Style-Bert-VITS2 も導入します。読み上げメニューは機能の ON/OFF のみを扱い、詳細な音声生成・学習ツールもアプリ内のツールメニューから `uv run` で起動します。
...
この Fork では macOS Intel をサポートしません。macOS は Apple Silicon のみ対応です。
...
- Linux x64: `uv run EasyNovelAssistant/setup/ena.py setup`
- macOS Apple Silicon: `uv run EasyNovelAssistant/setup/ena.py setup`

KoboldCpp は起動 OS に応じて `koboldcpp.exe`、`koboldcpp-linux-x64`、`koboldcpp-mac-arm64` を利用します。macOS は Apple Silicon のみ対応です。

Linux/macOS では `uv run EasyNovelAssistant/setup/ena.py setup` を実行してから `uv run EasyNovelAssistant/setup/ena.py run` で起動します。
